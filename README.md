...
- matrix 연산자 사용 예시 구현
  ...
  - matrix 연산자 멤버 객체(Clone, setTo)를 활용한 복제 (matOp2.cpp)
  - ROI(Region of Interest - 이미지 내 관심영역)을 활용한 이미지 내 특정 영역 추출 및 출력 (matOp3.cpp) - Rect 클래스 변수 활용(특정 초기 입력 좌표값을 통한 관심영역 지정)
  - 행렬 조작 기능 학습, 다양한 방법으로 행렬 접근 및 조작 - matrix 클래스(cv::Mat) 정적 멤버 함수 zeros(지정된 크기와 데이터 타입으로 모든 요소를 0으로 초기화하는 역할)사용, 이중 반복문 사용하여 각 픽셀을 반복하면서 해당 픽셀의 값을 1씩 증가('.at<Template>' method를 사용하여 요소에 접근, point 및 iteraotr 각각 활용) (matOp4.cpp)
  - 외부 저장 공간에 데이터 저장 및 출력 (matOp5.cpp)
  - 벡터(Vec3b 사용) 및 스칼라(Scalar) 행렬 객체를 활용한 색상 출력 (vecOp.cpp)